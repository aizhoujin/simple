layui.define(['utils', 'jquery', 'lodash', 'nprogress', 'layer'], function (exports) {
    ...
    Route.prototype.setRoutes = function (options) {
        ...
        options.routes.forEach(obj => {
            getRoute(defaults.routes, obj);
            //defaults.routes.push(r);
        });
        //options.routes.$.extend(true, defaults, options);
     ...
     * @param routes
     * @param route
     * @returns {{path: *, component: *, name: *}}
     */
    function getRoute(routes, route) {
        if (route.children !== undefined) {
            for (let i = 0; i < route.children.length; i++) {
                getRoute(routes, route.children[i]);
            }
        }

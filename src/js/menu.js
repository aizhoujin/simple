layui.define(['jquery', 'utils', 'axios', 'userApi'], function (exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    var $ = layui.jquery,
        utils = layui.utils,
        _ = layui.lodash,
        axios = layui.axios,
        userApi = layui.userApi
    ...
    Menu.prototype.render = function () {
        ...
        if (config.dynamicRender) {
            ...
            if (config.data.length > 0) {
                // 缓存数据
                storage.setItem(config.cacheKey, config.data);
                // 渲染DOM
    ...
    var _private = {
        ...
        recursion: function (temp, datas, pid) {
            ...
            _.forEach(datas, function (item, index) {
                if (item.pid === pid) {
                    //是菜单 才进行菜单操作
                    if (item.isMenu === undefined || item.isMenu === true) {
                        //有权限才进行操作
                        if (item.perm === undefined || userApi.getPerm(item.perm)) {
                            curr.push(item);
                        }
                    }
                }

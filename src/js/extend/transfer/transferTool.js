...
layui.define(['element', 'form',], function (exports) {
    ...
    let config = {
        ...
        url: undefined,  //url
        data: undefined,  //url
        codeCol: undefined,//code的对象字段名
        valueCol: undefined,//value的对象字段名
        codeName: undefined,
        enumName: undefined,
        name: "",  //form表单获取的结果值
    ...
    const transferTool = {
        ...
        renderData: function (R) {
            ...
            //编码字段
            const _codeCol = config.codeCol;
            //值字段
            const _valueCol = config.valueCol;
            ...
            if (_cascade === false) {
                _searchHtml = [
                    ...
                    '<input class="layui-input search_condition" style="width:130px;float:right;" placeholder="关键字搜索">',
            ...
            if (data !== undefined) {
                for (var i = 0; i < data.length; i++) {
                    //设置默认值(向右侧插入元素)
                    if (_values.indexOf(data[i][_codeCol]) == -1) {
                        var _input = '<dd lay-value="' + data[i][_codeCol] + '" lay-title="' + data[i][_valueCol] + '"><input type="checkbox" lay-filter="transferLeftChecked" title="' + data[i][_valueCol] + '" lay-skin="primary"></dd>';
                        //设置禁用
                        if (_disableds.indexOf(data[i][_codeCol]) != -1) {
                    ...
                    //像左侧插入元素
                        ...
                        var _input = '<dd lay-value="' + data[i][_codeCol] + '"  lay-title="' + data[i][_valueCol] + '"><input type="hidden" name="' + _name + '" value="' + data[i][_codeCol] + '"><input lay-filter="transferRightChecked"   type="checkbox"  title="' + data[i][_valueCol] + '" lay-skin="primary"></dd>';
                        //设置禁用
                        if (_disableds.indexOf(data[i][_codeCol]) != -1) {
                            _input = _input.replace("<input", "<input disabled ")
                        }
    ...
    layui.link(layui.cache.base + 'extend/transfer/cyType.css');

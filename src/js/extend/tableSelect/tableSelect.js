layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    tableSelect.prototype.render = function (opt) {
        const elem = $(opt.elem);
        const tableDone = opt.table.done || function () {
        };
        const before = opt.before || function (e) {
            return true;
        };
        ...
        elem.off('click').on('click', function (e) {
            ...
            const queryData = (query) => {
                opt.apiObj.selector(query)
                    .then((data) => {
                        if (data.code === 200) {
                            data_list = data.data;
                            opt.table.data = data_list.list;
                            opt.table.page = {
                                ...
                                jump: (obj, first) => {
                                    //首次不执行
                                    if (!first) {
                                        opt.query.page = obj.curr;
                                        opt.query.size = obj.limit;
                                        queryData(query);
                                    }
                                }
                            };
            ...
            //     const checkCell = obj.tr.find(".layui-form-checkbox");
            //     const radioCell = obj.tr.find(".layui-form-radio");
            //     if (obj.tr.hasClass('layui-table-click')) {
            //         obj.tr.removeClass('layui-table-click');
            //         checkCell.removeClass('layui-form-checked');
            //         // radioCell.removeClass('layui-form-radioed');
            //         radioCell.trigger("click");
            //     } else {
            //         obj.tr.addClass('layui-table-click');
            //         checkCell.addClass('layui-form-checked');
            //         // radioCell.addClass('layui-form-radioed');
            //         radioCell.trigger("click");
            //     }
            // });
            ...
            form.on('submit(tableSelect_btn_search)', function (data) {
                ...
                opt.query.like[opt.searchKey] = data.field[opt.searchKey];
                queryData(opt.query);
                return false;
            });
    ...
    layui.link(layui.cache.base + 'extend/tableSelect/style.css');
